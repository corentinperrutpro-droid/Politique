...
function runSearch() {
  ...
  ficheMatches.slice(0, 20).forEach(f => {
    ...
    const ficheIdJs = JSON.stringify(ficheId);

    html += `
      <a
        class="search-result"
        href="#fiche/${encodeURIComponent(ficheId)}"
        onclick="openFiche(${escapeHtml(ficheIdJs)})"
      >
        <span class="search-result-number">
...
function renderThemeDetail(num) {
  const t = SITE_DATA.themes.find(
    theme => Number(theme.num) === Number(num)
  );
        ...
        ${
          t.notionId
            ? `
              ·
              <a
                href="https://app.notion.com/p/${encodeURIComponent(t.notionId)}"
                target="_blank"
...
function renderFicheDetail(id) {
  ...
  const theme = SITE_DATA.themes
    .find(t => Number(t.num) === Number(f.theme));

  return `
    <div class="detail-breadcrumb">
      <a
        href="#theme/${f.theme}"
        onclick="openTheme(${Number(f.theme)})"
      >
        ← Retour au thème
        ${theme ? escapeHtml(theme.titre) : escapeHtml(f.theme)}
      </a>
    </div>
